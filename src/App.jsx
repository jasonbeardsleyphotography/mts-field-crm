...
if (geminiKey) {
  console.log('[TTS] Calling Gemini TTS...');
  try {
    ...
    console.warn("[TTS] Gemini failed:", e.message || e);
  }
}
...
if (window.speechSynthesis) {
  ...
  if (voices.length) {
    console.log('[TTS] Using speechSynthesis fallback'); window.speechSynthesis.speak(u);
  } else {
...
<style>{`.scr::-webkit-scrollbar{width:0} .gmnoprint,.gm-bundled-control,.gm-style-cc,.gm-control-active,.gm-fullscreen-control,.gm-style .adp,.gm-style button[title]{display:none!important} .mts-body{display:flex;flex-direction:column;flex:1;overflow:hidden} .mts-map{flex-shrink:0;border-bottom:1px solid #1a2030} .mts-list{flex:1;overflow-y:auto} @keyframes spin{to{transform:rotate(360deg)}} @media screen and (orientation:landscape){html{-webkit-text-size-adjust:100%}} @media(min-width:768px){ .mts-body{flex-direction:row} .mts-map{width:45%;min-width:320px;max-width:500px;border-bottom:none;border-right:1px solid #1a2030;overflow:hidden;display:flex;flex-direction:column} .mts-map .mts-map-inner{flex:1;min-height:0} .mts-map .mts-map-inner>div{height:100%!important} .mts-list{flex:1;min-width:0} .mts-pipeline-mobile{display:none!important} .mts-pipeline-desktop{display:flex!important} }`}</style>
