  ...
  return <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.7)", backdropFilter:"blur(4px)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
    <div onClick={e => e.stopPropagation()} style={{ background:"#0d0f14", border:"1px solid #1e2536", borderRadius:14, padding:18, maxWidth:480, width:"100%", maxHeight:"80vh", overflowY:"auto" }}>
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:14 }}>
        ...
        {stop.jn && <a href={`https://app.singleops.com/jobs?search=${stop.jn}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ padding:"3px 10px", borderRadius:5, fontSize:11, fontWeight:700, background:"#1a1e28", border:"1px solid #2a3040", color:"#6a7590", textDecoration:"none" }}>SO #{stop.jn} ↗</a>}
        <button onClick={onClose} style={{ width:30, height:30, borderRadius:8, background:"#111420", border:"1px solid #1e2536", color:"#6a7590", fontSize:16, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
      </div>
      ...
      <div style={{ background:"#0a0c10", borderRadius:10, padding:14, border:"1px solid #161b25" }}>
        <div style={{ fontSize:9, fontWeight:700, color:"#3a4560", letterSpacing:1, marginBottom:6 }}>EVENT DETAILS</div>
        <div style={{ fontSize:13, color:"#c0c8d8", lineHeight:1.5, marginBottom:8 }}>{stop.raw}</div>
      ...
      {log.length > 0 && <div style={{ marginTop:10, padding:"8px 10px", background:"#0a0c10", borderRadius:8, border:"1px solid #161b25" }}>
        ...
        {log.slice().reverse().map((e, i) => <div key={i} style={{ display:"flex", alignItems:"center", gap:6, padding:"3px 0", fontSize:11, color:"#6a7590" }}><span>{e.type==="call"?"📞":"💬"}</span><span style={{color:"#8a9ab0"}}>{e.type==="call"?"Called":"Texted"}</span><span style={{marginLeft:"auto",color:"#3a4560",fontSize:10}}>{e.ts}</span></div>)}
      </div>}
...
function SoldPicker({ stop, onConfirm, onCancel }) {
  ...
  return <div onClick={onCancel} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.7)", backdropFilter:"blur(4px)", zIndex:210, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
    <div onClick={e => e.stopPropagation()} style={{ background:"#0d0f14", border:`1px solid ${C.sage}44`, borderRadius:14, padding:22, maxWidth:360, width:"100%" }}>
      ...
      <input type="date" value={date} onChange={e => setDate(e.target.value)} style={{ width:"100%", padding:"10px 14px", borderRadius:8, border:`1px solid ${C.sage}33`, background:"#0a0c10", color:"#e0e4ea", fontSize:15, fontWeight:600, outline:"none", boxSizing:"border-box", colorScheme:"dark" }}/>
      <div style={{ display:"flex", gap:10, marginTop:18 }}>
        <button onClick={onCancel} style={{ flex:1, padding:"10px 0", borderRadius:8, background:"#111420", border:"1px solid #1e2536", color:"#6a7590", fontSize:13, fontWeight:600, cursor:"pointer" }}>Cancel</button>
  ...
  return <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.7)", backdropFilter:"blur(4px)", zIndex:205, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
    <div onClick={e => e.stopPropagation()} style={{ background:"#0d0f14", border:"1px solid #1e2536", borderRadius:"14px 14px 0 0", padding:18, maxWidth:480, width:"100%", maxHeight:"70vh", overflowY:"auto" }}>
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:14 }}>
        ...
        <button onClick={onClose} style={{ width:28, height:28, borderRadius:6, background:"#111420", border:"1px solid #1e2536", color:"#6a7590", fontSize:14, cursor:"pointer" }}>✕</button>
      </div>
      ...
      <input value={custom} onChange={e => setCustom(e.target.value)} placeholder="Or type custom..." style={{ width:"100%", padding:"10px 12px", borderRadius:8, border:"1px solid #1e2536", background:"#0a0c10", color:"#e0e4ea", fontSize:13, outline:"none", boxSizing:"border-box", marginTop:4 }}/>
...
export default function App() {
  ...
  // All pipeline events across ALL loaded days (for DeskPipe kanban)
  const allPipelineEvents = useMemo(() => {
    const seen = new Set();
    const all = [];
    Object.values(rawEvents).forEach(dayEvts => {
      dayEvts.forEach(ev => {
  ...
  const moveStage = async (stopId, toStage) => {
    if (toStage === "sage") { setSoldPick({ id:stopId, ...pmMap[stopId], prevStage:pmMap[stopId]?.ck }); return; }
    const prevCk = pmMap[stopId]?.ck;
  ...
  const InteractionHistory = ({stopId}) => { const log=interLog[stopId]; if(!log?.length)return null; return <div style={{marginTop:6,padding:"6px 8px",background:"#0a0c12",borderRadius:6,border:"1px solid #161b25"}}><div style={{fontSize:8,fontWeight:700,color:"#3a4560",letterSpacing:1,marginBottom:4}}>CONTACT LOG</div>{log.slice(-5).reverse().map((e,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"2px 0",fontSize:10,color:"#6a7590"}}><span>{e.type==="call"?"📞":"💬"}</span><span style={{color:"#8a9ab0"}}>{e.type==="call"?"Called":"Texted"}</span><span style={{marginLeft:"auto",color:"#3a4560"}}>{e.ts}</span></div>)}</div>; };
  ...
  if (!token) {
    return <div style={{ height:"100dvh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"#090b10", fontFamily:"Inter,-apple-system,sans-serif", color:"#e0e4ea", padding:20 }}>
      ...
      <button onClick={initAuth} style={{ padding:"14px 32px", borderRadius:10, background:"#1e2536", border:"1px solid #2a3548", color:"#e0e4ea", fontSize:15, fontWeight:700, cursor:"pointer" }}>Sign in with Google</button>
  ...
  return (
    <div ref={cRef} style={{height:"100dvh",width:"100%",background:"#090b10",display:"flex",flexDirection:"column",fontFamily:"'Inter',-apple-system,system-ui,sans-serif",color:"#e0e4ea",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:4,padding:"6px 8px",background:"#0d0f14",borderBottom:"1px solid #161b25",flexShrink:0,zIndex:20}}>
        ...
        {!searchOpen?<>
          <button onClick={()=>{setSearchOpen(true);setTimeout(()=>searchRef.current?.focus(),50);}} style={{width:28,height:28,borderRadius:6,background:"transparent",border:"1px solid #1e2536",color:"#3a4560",fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>🔍</button>
          <button onClick={undo} style={{width:28,height:28,borderRadius:6,background:undoStack.length?"#1e253666":"transparent",border:"1px solid #1e2536",color:undoStack.length?"#a0a8b8":"#1e2536",fontSize:12,cursor:undoStack.length?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}} disabled={!undoStack.length}>↩</button>
          <button onClick={loadEvents} style={{width:28,height:28,borderRadius:6,background:loading?"#1e253666":"transparent",border:"1px solid #1e2536",color:loading?"#039BE5":"#3a4560",fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}} title="Refresh">⟳</button>
        </>:
        <div style={{display:"flex",alignItems:"center",gap:3,flex:1,minWidth:0}}>
          <input ref={searchRef} value={searchQ} onChange={e=>setSearchQ(e.target.value)} onKeyDown={e=>{if(e.key==="Escape"){setSearchOpen(false);setSearchQ("");}}} placeholder="Name or address..." style={{flex:1,padding:"5px 8px",borderRadius:6,border:"1px solid #1e2536",background:"#0a0c12",color:"#e0e4ea",fontSize:12,outline:"none",minWidth:0}}/>
          <button onClick={()=>{setSearchOpen(false);setSearchQ("");}} style={{padding:"4px 6px",borderRadius:5,background:"transparent",border:"none",color:"#6a7590",fontSize:10,fontWeight:700,cursor:"pointer",flexShrink:0}}>✕</button>
        </div>}
        {!searchOpen&&<div style={{flex:1}}/>}
        {!searchOpen&&<div style={{display:"flex",gap:2,background:"#0a0c12",borderRadius:7,padding:2}}>
          {[{k:"route",l:"Route"},{k:"pipeline",l:"Pipeline"},{k:"deskpipe",l:"DeskPipe"}].map(v=><button key={v.k} onClick={()=>{setView(v.k);setActStop(null);setMoving(null);setReorderMode(false);}} style={{padding:"4px 10px",borderRadius:5,border:"none",background:view===v.k?"#1e2536":"transparent",color:view===v.k?"#fff":"#3a4560",fontSize:12,fontWeight:600,cursor:"pointer",position:"relative"}}>{v.l}{v.k==="pipeline"&&syncQueue.length>0&&<span style={{position:"absolute",top:-2,right:-2,width:14,height:14,borderRadius:7,background:"#F4511E",color:"#fff",fontSize:8,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center"}}>{syncQueue.length}</span>}</button>)}
        </div>}
        <button onClick={()=>setShowLeg(!showLeg)} style={{width:28,height:28,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",background:showLeg?"#1e2536":"transparent",border:"1px solid #1e2536",color:"#3a4560",fontSize:12,cursor:"pointer",flexShrink:0}}>🎨</button>
      ...
      <div style={{background:"#0b0d12",borderBottom:"1px solid #161b25",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",padding:"4px 10px",gap:5}}>
          <button onClick={()=>setTodoOpen(!todoOpen)} style={{background:"none",border:"none",color:"#3a4560",fontSize:11,cursor:"pointer",fontWeight:700,letterSpacing:1,display:"flex",alignItems:"center",gap:4}}>
          ...
          <div style={{flex:1}}/>
          {!todoInput?<button onClick={()=>{setTodoInput(true);setTodoOpen(true);setTimeout(()=>todoRef.current?.focus(),50);}} style={{width:22,height:22,borderRadius:5,background:"transparent",border:"1px dashed #1e2536",color:"#3a4560",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>:
          <div style={{display:"flex",gap:4}}>
            <input ref={todoRef} value={todoText} onChange={e=>setTodoText(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")addTodo();if(e.key==="Escape"){setTodoInput(false);setTodoText("");}}} placeholder="TODO..." style={{padding:"3px 8px",borderRadius:5,border:"1px solid #1e2536",background:"#111420",color:"#e0e4ea",fontSize:12,width:160,outline:"none"}}/>
      ...
      {view === "route" ? (
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
                ...
                <div onClick={()=>{if(reorderMode)handleTap(idx);else setActStop(isA?null:idx);}} style={{padding:isNext?"12px 14px":"10px 14px",borderBottom:"1px solid #0e1018",cursor:reorderMode?"grab":"pointer",background:isMov?"#1a0f28":reorderMode?"#0a0c12":isNext?"#0d1520":isA?"#0f1520":"transparent",borderLeft:`4px solid ${isMov?"#8E24AA":isNext?C[s.ck]:isA&&!reorderMode?C[s.ck]:"transparent"}`,opacity:reorderMode&&!isMov&&moving!==null?.7:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:isNext?16:15,fontWeight:isNext?700:600,color:"#e0e4ea",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{isNext?"▸ ":""}{s.cn}</div>
                  ...
                  {isA&&!reorderMode&&<div style={{marginTop:10,marginLeft:40,paddingTop:10,borderTop:"1px solid #161b25"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:8,flexWrap:"wrap"}}>
                      ...
                      {s.jn&&<a href={`https://app.singleops.com/jobs?search=${s.jn}`} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{padding:"2px 8px",borderRadius:5,fontSize:10,fontWeight:700,background:"#1a1e28",border:"1px solid #2a3040",color:"#6a7590",textDecoration:"none"}}>#{s.jn} ↗</a>}
          ...
          {completedStops.length>0&&<div style={{borderTop:"1px solid #161b25"}}>
            ...
            {completedOpen&&completedStops.map(s=><div key={s.id} style={{padding:"8px 14px",borderBottom:"1px solid #0a0c10",opacity:.5,display:"flex",alignItems:"center",gap:10}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:13,color:"#6a7590",textDecoration:"line-through"}}>{s.cn}</div></div>
              <button onClick={()=>undismissStop(s.id)} style={{padding:"3px 8px",borderRadius:5,background:"transparent",border:"1px solid #1e2536",color:"#6a7590",fontSize:9,fontWeight:600,cursor:"pointer"}}>Restore</button>
            </div>)}
          </div>}
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          {syncQueue.length>0&&<div style={{padding:"8px 10px",background:"#1a0a0a",borderBottom:"1px solid #3a1a1a",flexShrink:0}}>
            ...
            {syncQueue.map((q,qi)=><div key={q.id+qi} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 8px",background:"#111420",borderRadius:6,marginBottom:3,border:"1px solid #1e2536"}}>
              <Dot color={q.stage} sz={8}/><div style={{flex:1,minWidth:0}}><span style={{fontSize:12,fontWeight:600,color:"#c0c8d8"}}>{q.cn}</span><span style={{fontSize:10,color:"#F4511E",marginLeft:6,fontWeight:600}}>{q.action}</span></div>
              {q.jn&&<a href={`https://app.singleops.com/jobs?search=${q.jn}`} target="_blank" rel="noopener noreferrer" style={{padding:"3px 8px",borderRadius:4,fontSize:10,fontWeight:700,background:"#1a1e28",border:"1px solid #2a3040",color:"#6a7590",textDecoration:"none",flexShrink:0}}>SO #{q.jn} ↗</a>}
          ...
          {needsAttention.length>0&&<div style={{padding:"8px 10px",background:"#1a1410",borderBottom:"1px solid #3a2a1a",flexShrink:0}}>
            ...
            <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4}}>
              {needsAttention.slice(0,5).map(s=><div key={s.id} onClick={()=>{setPipeStage(s.ck);setExpandedPipe(s.id);}} style={{flexShrink:0,padding:"6px 10px",borderRadius:8,background:"#111420",border:`1px solid ${C[s.ck]}33`,cursor:"pointer",minWidth:180}}>
                <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:3}}><Dot color={s.ck} sz={6}/><span style={{fontSize:11,fontWeight:600,color:"#e0e4ea"}}>{s.cn}</span><span style={{marginLeft:"auto",fontSize:9,color:"#c85a5a",fontWeight:700}}>{s.age}</span></div>
          ...
          <div style={{display:"flex",padding:"6px 8px",gap:3,overflowX:"auto",flexShrink:0,background:"#0b0d12",borderBottom:"1px solid #161b25"}}>
            {PO.map(k=>{const cnt=todayP.filter(s=>s.ck===k).length;return <button key={k} onClick={()=>{setPipeStage(k);setExpandedPipe(null);}} style={{padding:"5px 10px",borderRadius:6,border:`1px solid ${pipeStage===k?C[k]+"55":"#161b25"}`,background:pipeStage===k?C[k]+"15":"transparent",color:pipeStage===k?C[k]:"#4a5a6a",fontSize:10,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:4,flexShrink:0}}><Dot color={k} sz={6}/>{PL[k].split(" ")[0]} <span style={{fontWeight:800}}>{cnt}</span></button>;})}
          </div>
          ...
          <div className="scr" style={{flex:1,overflowY:"auto",padding:"10px 10px max(10px, env(safe-area-inset-bottom))"}}>
            {(()=>{
              ...
              if(items.length===0)return <div style={{textAlign:"center",padding:40,color:"#1e2536",fontSize:13}}>{pipeSearch?`No matches`:`No items in ${PL[pipeStage]}`}</div>;
              return items.map(s=>{
                ...
                return <div key={s.id} onClick={()=>setExpandedPipe(isExp?null:s.id)} style={{padding:"12px 14px",background:"#0d0f14",borderRadius:10,border:`1px solid ${isAging?"#4a3a1a":"#161b25"}`,marginBottom:6,borderLeft:`4px solid ${C[pipeStage]}`,cursor:"pointer"}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:600,color:"#c0c8d8"}}>{s.cn}</div><div style={{fontSize:12,color:"#4a5a6a"}}>{s.addr}</div></div>
                  ...
                  {s.jn&&<a href={`https://app.singleops.com/jobs?search=${s.jn}`} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{display:"inline-block",marginTop:4,fontSize:10,fontWeight:700,color:"#6a7590",textDecoration:"none",padding:"2px 8px",borderRadius:4,background:"#0a0c12",border:"1px solid #161b25"}}>SO #{s.jn} ↗</a>}
        ...
        /* ═══ DESKPIPE KANBAN ═══ */
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{padding:"6px 10px",fontSize:10,fontWeight:700,color:"#3a4560",letterSpacing:1,borderBottom:"1px solid #161b25",background:"#0b0d12",flexShrink:0,display:"flex",alignItems:"center",gap:8}}>
            <span>PIPELINE KANBAN</span>
            <span style={{fontWeight:400,color:"#2a3548"}}>·</span>
            <span style={{fontWeight:400,color:"#4a5a6a"}}>{allPipelineEvents.length} total across {Object.keys(rawEvents).length} days</span>
          </div>
          <div style={{flex:1,display:"flex",overflowX:"auto",overflowY:"hidden",gap:0,background:"#070910"}}>
            {PO.map(stageKey => {
              const stageItems = allPipelineEvents.filter(s => s.ck === stageKey);
              return <div key={stageKey}
                ...
                <div className="scr" style={{flex:1,overflowY:"auto",padding:6}}>
                  {stageItems.length === 0 && <div style={{padding:"20px 8px",textAlign:"center",color:"#1e2536",fontSize:11}}>No items</div>}
                  {stageItems.map(s => (
                    ...
                    >
                      <div style={{fontSize:13,fontWeight:600,color:"#c0c8d8",marginBottom:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.cn}</div>
                      {s.addr && <div style={{fontSize:10,color:"#4a5a6a",marginBottom:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.addr}</div>}
                      <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
                        {s.db && <span style={{fontSize:9,color:WS.driveby.c,fontWeight:700}}>🚗 DB</span>}
                        {s.jn && <a href={`https://app.singleops.com/jobs?search=${s.jn}`} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{fontSize:9,fontWeight:700,color:"#6a7590",textDecoration:"none",padding:"1px 5px",borderRadius:3,background:"#111420",border:"1px solid #1e2536"}}>#{s.jn}</a>}
                        <span style={{fontSize:9,color:s.ageDays>5?"#c85a5a":"#3a4560",fontWeight:600,marginLeft:"auto"}}>{s.age}</span>
                      </div>
